...
4. Run `./scripts/verify-cloud.sh` after each integration.
5. For full E2E, unset external `KUBECONFIG` and run `./scripts/verify-local.sh`; it establishes the isolated pre-creation/existing-cluster guard.
6. Before any additional direct `kubectl` command, run `./scripts/kind-e2e-preflight.sh` against the created disposable cluster.
