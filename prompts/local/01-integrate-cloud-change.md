...
5. Run `./scripts/verify-cloud.sh` after integration.
6. Explicitly spawn `code_reviewer` and `test_engineer`; add `security_reviewer` when relevant.
7. For Kubernetes-sensitive milestones, prepare the next local E2E step but do not run kubectl until `./scripts/kind-e2e-preflight.sh` proves a disposable context.
