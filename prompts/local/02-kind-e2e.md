...
3. Refuse an external kubeconfig: `unset KUBECONFIG`, then run `bash scripts/verify-local.sh`. This script establishes an isolated kubeconfig and performs the safe existing-or-missing-cluster preflight before the reviewed E2E target creates or uses the cluster.
4. Before any additional direct `kubectl` command, run `bash scripts/kind-e2e-preflight.sh` and preserve its context evidence.
